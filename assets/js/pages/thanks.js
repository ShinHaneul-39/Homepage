...
class ThanksManager {
  ...
  /**
   * CSV 데이터 로드 및 파싱 (캐싱 적용)
   * localStorage를 활용한 Stale-While-Revalidate 전략 적용
   */
  loadData() {
      if (typeof Papa === 'undefined') return;

      const CACHE_KEY = 'thanks_data_v1';
      ...
      if (cachedData) {
          try {
              ...
              // 백그라운드 갱신 (렌더링 안함)
              this.fetchData(CACHE_KEY, false); 
              return;
          } catch (e) {
      ...
      // 2. 캐시 없으면 네트워크 로드
      this.fetchData(CACHE_KEY, true);
  }

  /**
   * 실제 네트워크 요청 수행
   * @param {string} cacheKey - 캐시 키
   * @param {boolean} shouldRender - 렌더링 여부 (백그라운드 갱신 시 false)
   */
  fetchData(cacheKey, shouldRender) {
      fetch('../assets/data/thanks_data.csv?t=' + Date.now())
          .then(r => r.text())
          .then(csvText => {
              Papa.parse(csvText, {
                  ...
                  complete: (results) => {
                      if (results.data && results.data.length > 0) {
                          ...
                          if (shouldRender) {
                              ...
                              console.log('Thanks data cache updated in background');
                          }
                      }
                  }
              });
